    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.list}>
      {drivers.map(({ driver }, index) => {
        ...
        return <Pressable key={driver.driver_number} accessibilityRole="button" accessibilityState={{ selected: active }}
          ...
          <Text style={[styles.position, active && { color: teamColor }]}>P{index + 1}</Text>
...
const styles = StyleSheet.create({
  ...
  position: { position: 'absolute', top: 3, right: 5, color: colors.textDim, fontFamily: 'BarlowCondensed_600SemiBold', fontSize: 8 },
